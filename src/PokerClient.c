...
///////////////////
  ...
  char bank1Val, bank2Val, bank3Val, bank4Val, bank5Val;
  
  char potVal;
  
  //char betVal1, betVal2, betVal3, betVal4, betVal4;
  
 /* GtkWidget *bank1 = gtk_label_new(bank1Val);
  GtkWidget *bank2 = gtk_label_new(bank2Val);
  GtkWidget *bank3 = gtk_label_new(bank3Val);
  GtkWidget *bank4 = gtk_label_new(bank4Val);
  GtkWidget *bank5 = gtk_label_new(bank5Val);
  //set size and fixedput
  GtkWidget *pot = gtk_label_new(potVal);
  //set size and fixedput
  GtkWidget *bet1 = gtk_label_new(bet1Val);
  GtkWidget *bet2 = gtk_label_new(bet2Val);
  GtkWidget *bet3 = gtk_label_new(bet3Val);
  GtkWidget *bet4 = gtk_label_new(bet4Val);
  GtkWidget *bet5 = gtk_label_new(bet5Val);*/
  //set size and fixedput
  ...
  i_bg3 = gtk_image_new_from_file("/users/ugrad2/2017/winter/team9/vanessa/images/1000 px game background.png");
  gtk_fixed_put(GTK_FIXED(game_menu), i_bg3, 0, 0);
  
 int div = 14;	
 /* gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,0), 279, 355); 
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 356+div, 355);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 356+(2*div)+ 78, 355);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 356+(3*div)+ 2*78, 355);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 356+(4*div)+ 3*78 + 1, 355);
  
  //player 1 cards
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 31, 38);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 31 + div + 78, 38);
  //player 2 cards
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 323 + div + 78, 38);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 324 + 2*div + 2*78, 38);
  //player 3 cards
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 323 + 2*div + 2*78 + 292, 38);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 324 + 3*div + 3*78 + 292, 38);
  //player 4 cards
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 31, 607);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 32 + div + 78, 607);
  //player 5 cards
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 323 + 2*div + 2*78 + 292, 606);
  gtk_fixed_put(GTK_FIXED(game_menu), CardImagePicker(s,3), 324 + 3*div + 3*78 + 292, 606);
  */
  //buttons
  gtk_fixed_put(GTK_FIXED(game_menu), b_check, bg_width/6 - button_width/6 + 62, bg_height/2 + 220);
  gtk_fixed_put(GTK_FIXED(game_menu), b_call, bg_width/6 - button_width/6 + 179, bg_height/2 + 220);
  gtk_fixed_put(GTK_FIXED(game_menu), b_raise, bg_width/6 - button_width/6 + 297, bg_height/2 + 220);
  gtk_fixed_put(GTK_FIXED(game_menu), b_allin, bg_width/6 - button_width/6 + 416, bg_height/2 + 220);
  gtk_fixed_put(GTK_FIXED(game_menu), b_fold, bg_width/6 - button_width/6 + 532, bg_height/2 + 220);
  gtk_fixed_put(GTK_FIXED(game_menu), b_leaveGame, bg_width/6 - button_width/6 + 217, bg_height/2 + 297);
...
char P0Bank[5]="";
	...
	*Server;	/* server host */
    ...
    {	printf("%s: Enter a command to send to the poker server:\n"
		...
		"         'BOT' to play poker game with a robot,\n"
	...
	while(0 != strcmp("FOLD", SendBuf))
	{
		...
		var0Bet();
		length=sizeof(P0Bet)-1;//calculate length of array
		memset(P0Bet, 0, length); //set all bytes in array to 0
		x = strlen(SendBuf);
	        if (SendBuf[x-1] == '\n')
		{   SendBuf[--x] = 0;
		}
		...
		var0Bank();
		length=sizeof(P0Bank)-1;//calculate length of array
		memset(P0Bank, 0, length); //set all bytes in array to 0
		x = strlen(SendBuf);
	        if (SendBuf[x-1] == '\n')
		{   SendBuf[--x] = 0;
		}
