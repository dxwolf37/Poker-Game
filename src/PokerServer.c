...
void DisplayPlayer0Bet() //display player's bet
{
	
	printf("|P0 Bet:%d", Player0Bet); //prints current players bet on server
	...
	int DataSocketFD)
{
   ...
   /* srand(time(NULL));  
    ...
    else if (0 == strcmp(RecvBuf, "BOT"))
    {
	NumPlayers+=1;
	//set bot 
	Bot=1;
	Player0Bet=SmallBlind;
	P0Bank-=Player0Bet;
	PlayerBank=P0Bank;
	//PlayerBet=Player0Bet;
	CurrentBet=Player0Bet;
	Give2Cards(D, C, P0);
	...
	//memset(SendBuf,0,sizeof(RecvBuf)); //clear send buffer
	memset(SendBuf,0,sizeof(SendBuf)); //clear send buffer
	pot=Player1Bet+Player2Bet+Player3Bet+Player4Bet+Player5Bet+Player0Bet;
	//char ClientOutput[100]; //placeholder var to hold output to client
    ...
    else if(0 == strcmp(RecvBuf, "0BANK"))
    {
	memset(SendBuf,0,sizeof(SendBuf)); //clear send buffer
	char sP0Bank[10];//char ptr that holds the house cards 
	sprintf(sP0Bank, "%d", P0Bank); //copies char of housecards to output array  
	strcat(ClientOutput,sP0Bank);
	    
	//sends bank array to client
	strncpy(SendBuf, "\nBot's Bank:  \n", sizeof(SendBuf)-1);
	SendBuf[sizeof(SendBuf)-1] = 0;
	strncat(SendBuf, ClientOutput, sizeof(SendBuf)-1-strlen(SendBuf));
    ...
    else if (0 == strcmp(RecvBuf, "0BET"))
    {
	//memset(SendBuf,0,sizeof(RecvBuf)); //clear send buffer
	memset(SendBuf,0,sizeof(SendBuf)); //clear send buffer
	//char ClientOutput[100]; //placeholder var to hold output to client
	char sPlayer0Bet[10];//char ptr that holds the house cards 
	sprintf(sPlayer0Bet, "%d", Player0Bet); //copies char of housecards to output array  
	strcat(ClientOutput,sPlayer0Bet);
...
//END OF BETS TO CLIENT
	...
	if (Bot==1)
	{
		...
		{	
			//bot will call
			difference=CurrentBet-Player0Bet;
			Player0Bet=CurrentBet;
			P0Bank-=difference;
			PlayerBank=P0Bank;			
			//turn maintenance
			turn=1;
			if (cardsGiven<2) //if 2 card not given to house
			{
	...
	else
	{
		...
		{	
			...
			if (cardsGiven<2) //if 2 cards not given to house
			{
				Give1CardtoHouse(D, C, H);
				cardsGiven+=1; //update cards given var
				turn=1;
